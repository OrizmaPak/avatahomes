let trialbalanceid
async function trialbalanceActive() {
    ...
    if(request.status) {
        if(!id){
            if(request.data.length) {
                ...
                resolvePagination(datasource, ontrialbalanceTableDataSignal)
            }
        }else{ 
             trialbalanceid = request.data[0].id
...
async function removetrialbalance(id) {
    // Ask for confirmation
    const confirmed = window.confirm("Are you sure you want to remove this trialbalance?");
...
async function ontrialbalanceTableDataSignal() {
    ...
    <tr>
        ...
        <td class="flex items-center gap-3">
            <button title="Edit row entry" onclick="fetchtrialbalance('${item.id}')" class="material-symbols-outlined rounded-full bg-primary-g h-8 w-8 text-white drop-shadow-md text-xs" style="font-size: 18px;">edit</button>
            <button title="Delete row entry"s onclick="removetrialbalance('${item.id}')" class="material-symbols-outlined rounded-full bg-red-600 h-8 w-8 text-white drop-shadow-md text-xs" style="font-size: 18px;">delete</button>
        </td>
    </tr>`
    )
    .join('')
...
async function trialbalanceFormSubmitHandler() {
    ...
    document.querySelector('#trialbalanceform').reset();
    // fetchtrialbalance();
    return notification(request.message, 0);
}


// function runAdtrialbalanceFormValidations() {
//     let form = document.getElementById('trialbalanceform')
//     let errorElements = form.querySelectorAll('.control-error')
//     let controls = []

//     if(controlHasValue(form, '#owner'))  controls.push([form.querySelector('#owner'), 'Select an owner'])
//     if(controlHasValue(form, '#trialbalancename'))  controls.push([form.querySelector('#trialbalancename'), 'trialbalance name is required'])
